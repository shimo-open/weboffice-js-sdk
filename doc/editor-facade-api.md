...
- docs 标准化入口：`sdk.ActiveDocument.Editor.Document`、`sdk.ActiveDocument.Reference`、`sdk.ActiveDocument.Service`、`sdk.ActiveDocument.Sub`、`sdk.ActiveDocument.Env`
...
| ------------ | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| docs         | 标准化产品对象树                                | `sdk.ActiveDocument.Editor.*`、`sdk.ActiveDocument.Reference.*`、`sdk.ActiveDocument.Service.*`、`sdk.ActiveDocument.Sub.*`、`sdk.ActiveDocument.Env.*` | receiver `productJSAPI.*`；只在 docs 挂载                                             |
...
- `SheetSelection.id` 是当前 iframe runtime 内稳定的公开标识；内部 `sheetId`、`selectionId` locator 不直接暴露。
- 部分额外已承接能力，如 `history / locks / mention / content / version`，仍继续挂在根级 facade。

```typescript
const sdk = await connect(options)

const snapshot = await sdk.ActiveDocument?.Editor.Document.GetContent()
await sdk.ActiveDocument?.Editor.Document.Font.SetBold(true)
...
| 方法                                                     | 说明                             | 平台      |
| -------------------------------------------------------- | -------------------------------- | --------- |
| `sdk.ActiveDocument?.*`                                  | docs 标准化强类型产品对象树      | `PC only` |
...
- `sdk.ActiveDocument?.Editor.Document.*`
- `sdk.ActiveDocument.Reference.*`
- `sdk.ActiveDocument.Service.*`
- `sdk.ActiveDocument.Sub.OnDocumentChange(handler)`
- `sdk.ActiveDocument.Env.*`
